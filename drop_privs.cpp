...
gid_t groups[GROUP_SIZE] = { 1001, 1002 };
...
void log_resugid() {
    ...
    std::cout << "ruid " << ruid << " euid " << euid << " suid " << suid << std::endl;
    std::cout << "rgid " << rgid << " egid " << egid << " sgid " << sgid << std::endl;

    int nsupp_groups = getgroups(0, NULL);
    if (nsupp_groups < 0) {
        std::cerr << "getgroups(0)" << std::endl;
    }
    if (nsupp_groups == 0) {
        std::cerr << "no supplemental groups" << std::endl;
        return;
    }
    ...
    if (nsupp_groups < 0) {
        std::cerr << "getgroups(nsupp_groups)" << std::endl;
    }
    for (size_t i = 0; i < (size_t)nsupp_groups; i++) {
        std::cout << "supp gid " << i + 1 << " " << list[i] << std::endl;
    }
    free(list);
}
...
int main(void) {
    ...
    minijail_set_supplementary_gids(j, GROUP_SIZE, groups);
